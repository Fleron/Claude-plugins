...
The **Pragmatic Rust Guidelines** serve as a comprehensive collection of best practices designed to help developers write high-quality, maintainable, and idiomatic Rust code. A unique aspect of these guidelines is that they are written with **AI agents** in mind; by creating APIs that are easier for humans to understand, developers simultaneously maximize the utility of AI tools working within their codebases. The document emphasizes that idiomatic patterns and thorough documentation allow agents to iterate quickly and perform refactoring in a mostly "hands-off" mode.

The guidelines are organized into several specialized sections covering **application design, documentation, performance, safety, and library UX**. For documentation, it mandates canonical sections like "Errors" and "Panics" and requires skimmable summary sentences of fewer than 15 words. In terms of library design, it encourages the use of **inherent functions** over traits for essential functionality and provides a "design escalation ladder" that prioritizes concrete types over generics or dynamic traits to reduce cognitive load. It also provides strict rules for **error handling**, advocating for situation-specific structs in libraries while allowing simplified crates like `anyhow` for applications.

The intended use of these guidelines is to ensure that Rust projects are **performant, resilient, and "correct by construction"**. They provide actionable advice on identifying hot paths for optimization, isolating state in FFI libraries to prevent undefined behavior, and using static verification tools like **Clippy, Miri, and cargo-audit** to maintain code quality. By following these standards, developers can create libraries that "just work" out of the box across all platforms while ensuring that public APIs remain flexible and easy to use through patterns like builders and mockable system calls
