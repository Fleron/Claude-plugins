...
1. Run `gh auth status` and stop with a clear blocked state if `gh` is missing or unauthenticated.
2. Push the current branch.
3. Create a PR with `gh pr create`.
4. Update the active goal metadata to `workflow_stage: implementation_pr_open`, `implementation_pr_url`, and `implementation_pr_number`.
5. Stop and state that human review plus PR merge is the acceptance gate.

### Step 1a: Verify Tests
...
### Step 1b: Verify lint and formatting
...
**If formatting and linting fail:**
First define each failure based on if it is critical (e.g. clippy warnings treated as errors) or non-critical (e.g. go fmt issues that can be auto-fixed).
