---
name: babysit-pr
description: "Use when the user asks to monitor, watch or babysit a pull request, keep an eye on CI, handle incoming review comments, or stay on top of failures and feedback on an open PR until it is merged or closed."
---

# PR Babysitter

Ported from the Codex `babysit-pr` skill. Vendor-neutral: wherever the original named its agent, read "the agent" as whichever agent runs this skill. Substitute the agent's name where a prefix is called for (`claude:` in Claude Code, `codex:` in Codex).

## Vendor notes

- **Watcher path.** The script lives next to this file at `scripts/gh_pr_watch.py`. In Claude Code the plugin root is `${CLAUDE_PLUGIN_ROOT}`, so the full path is `${CLAUDE_PLUGIN_ROOT}/skills/babysit-pr/scripts/gh_pr_watch.py`. In Codex the equivalent is the skill's own directory under `.codex/skills/`.
- **Long-running `--watch`.** Codex keeps a foreground process streaming in the terminal. In Claude Code a foreground shell command times out after 10 minutes, so run `--watch` as a background Bash task and consume its stream with `Monitor` or `TaskOutput`, or drive `--once` from `/loop 1m`. Either way the rule below stands: one watcher per PR, keep consuming its output, do not end the turn while it runs.
- **Prefixes.** Commit messages, approved review replies and thread-resolution comments carry the agent's name so humans can tell agent activity from the user's. Use `claude:` / `[claude]` / `[from Claude]: ` in Claude Code, `codex:` / `[codex]` / `[from Codex]: ` in Codex.
- **Bots.** The watcher surfaces approved review bots whose login contains `codex` or `claude` (for example `chatgpt-codex-connector[bot]`, `claude[bot]`) alongside trusted humans.
...
`WATCHER` below stands for the watcher path from the vendor notes.

### One-shot snapshot

```bash
python3 "$WATCHER" --pr auto --once
```

### Continuous watch

```bash
python3 "$WATCHER" --pr auto --watch
```

### Trigger flaky retry cycle (only when watcher indicates)

```bash
python3 "$WATCHER" --pr auto --retry-failed-now
```

### Explicit PR target

```bash
python3 "$WATCHER" --pr <number-or-url> --once
```
...
- `gh api repos/<owner>/<repo>/actions/jobs/<job-id>/logs > "${TMPDIR:-/tmp}/gh-job-<job-id>-logs.zip"`
...
It intentionally surfaces approved review bot feedback (see vendor notes) in addition to human reviewer feedback. Most unrelated bot noise should still be ignored.
...
2. Commit with `<agent>: address PR review feedback (#<n>)`.
...
You can resolve review comment threads from the human who requested babysitting or from an approved
...
- `<agent>: fix CI failure on PR #<n>`
- `<agent>: address PR review feedback (#<n>)`
...
- GitHub CLI/API details used by the watcher: [references/github-api-notes.md](references/github-api-notes.md)
- Upstream source: https://github.com/openai/codex/tree/main/.codex/skills/babysit-pr
